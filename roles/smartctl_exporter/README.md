...
Role Variables
--------------
The exporter will scan the system for available devices if no `--smartctl.device` flags are used. The format of web.config file id described [here](https://github.com/prometheus/exporter-toolkit/blob/master/docs/web-configuration.md). If `smartctl_exporter_web_conf` is defined, role searches for `web.conf` file and copy it to target host to `smartctl_exporter_web_conf` directory
